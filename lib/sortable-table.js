...
export default class SortableTable extends React.Component<Props, State> {
  ...
  getCellColumnProps(node: React.Element, index: number): Object {
    return {
      ...node.props,
      key: node.key || index,
      'data-index': index,
  ...
  handleDragStart = (e: SyntheticDragEvent<HTMLElement>) => {
    ...
    const columns: React.Element = this.renderChildren(this.props.children, index);
  ...
  renderCell = (node: React.Element, index: number): React.Element => {
  ...
  renderRow = (row: React.Element, index: number = 0, column: number): React.Element => {
    let children = React.Children.toArray(row.props.children);

    if (this.state.reorder) {
      const { oldIndex, newIndex } = this.state.reorder;

      children = move(children, oldIndex, newIndex);
    }
    ...
    return (
      ...
      >
        {column === -1
          ...
          : this.renderCell(children[column], 0)}
      </ProgressiveTable.Row>
    );
  };
